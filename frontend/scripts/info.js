...
database.fetchData(`users/${userID}`).then(res => {
    if (res?.message === "Could not find user" || res === undefined) {
        ...
        greetText.innerText = "Welcome, Guest"
        return
    }
    ...
    if (res?.data.income.length > 0) {
        ...
        res?.data.income.forEach((income, index) => {
            const cont = elements.createInfoElement("income", "+", formatCurrency(income), "income", "INCOME", "in", () => {
                if (!confirm("Delete this income entry?")) return;
                database.postData("users/income/delete", { id: userID, index }).then(delRes => {
                    if (delRes?.message !== "Income deleted successfully") {
                        elements.showToast(delRes?.message || "Could not delete income", "error");
                        return;
                    }
                    elements.showToast("Income deleted", "success");
                    setTimeout(() => window.location.reload(), 500);
                }).catch(() => {
                    elements.showToast("Server error, please try again", "error");
                })
            })
    ...
    if (res?.data.expenses.length > 0) {
        ...
        res?.data.expenses.forEach(expense => {
            const cont = elements.createInfoElement("expenses", "-", formatCurrency(expense.amount), `${expense.expenseType.toLowerCase()}`, `${expense.expenseType.toUpperCase()}`, "out", () => {
                if (!confirm("Delete this expense entry?")) return;
                database.postData("users/expenses/delete", { id: userID, expenseId: expense._id }).then(delRes => {
                    if (delRes?.message !== "Expense deleted successfully") {
                        elements.showToast(delRes?.message || "Could not delete expense", "error");
                        return;
                    }
                    elements.showToast("Expense deleted", "success");
                    setTimeout(() => window.location.reload(), 500);
                }).catch(() => {
                    elements.showToast("Server error, please try again", "error");
                })
    ...
    // Populate "Last Expense" sidebar card
    const lastExpenseContent = document.querySelector(".last_expense_content");
    if (res?.data.expenses.length > 0) {
        const lastExpense = res.data.expenses[res.data.expenses.length - 1];
        const typeClass = lastExpense.expenseType.toLowerCase();
        lastExpenseContent.innerHTML = "";
        const row = document.createElement("div");
        row.classList.add("last_expense_row");
        row.innerHTML = `
            <div class="last_expense_info">
                <div class="last_expense_circle ${typeClass}">out</div>
                <div>
                    <p class="last_expense_type">${lastExpense.expenseType}</p>
                    <p class="last_expense_label">Most recent expense</p>
                </div>
            </div>
            <p class="last_expense_amount">-₹${formatCurrency(lastExpense.amount)}</p>
        `;
        lastExpenseContent.appendChild(row);
    }

    // Populate "Recent Activity" sidebar card
    const historyList = document.querySelector(".history_list");
    const recentIncome = res?.data.income.slice(-3).map(amount => ({
        label: "Income", amount, sign: "+", cls: "in", dotClass: "income"
    }));
    const recentExpenses = res?.data.expenses.slice(-3).map(expense => ({
        label: expense.expenseType, amount: expense.amount, sign: "-", cls: "out", dotClass: expense.expenseType.toLowerCase()
    }));
    const combinedHistory = [...(recentIncome || []), ...(recentExpenses || [])].slice(-5).reverse();

    if (combinedHistory.length > 0) {
        historyList.innerHTML = "";
        combinedHistory.forEach(item => {
            const row = document.createElement("div");
            row.classList.add("history_item");
            row.innerHTML = `
                <span class="history_item_label">
                    <span class="history_dot ${item.dotClass}"></span>
                    ${item.label}
                </span>
                <span class="history_item_amount ${item.cls}">${item.sign}₹${formatCurrency(item.amount)}</span>
            `;
...
getSelectBox.addEventListener("click", (e) => {
    e.stopPropagation();
    getOptionsContainer.style.display = getOptionsContainer.style.display === "flex" ? "none" : "flex";
})
...
getExpenseSelectBox.addEventListener("click", (e) => {
    e.stopPropagation();
    getExpenseOptionsContainer.style.display = getExpenseOptionsContainer.style.display === "flex" ? "none" : "flex";
})
...
document.addEventListener("click", (e) => {
    if (!getSelectBox.contains(e.target) && !getOptionsContainer.contains(e.target)) {
        getOptionsContainer.style.display = "none";
    }
    if (!getExpenseSelectBox.contains(e.target) && !getExpenseOptionsContainer.contains(e.target)) {
        getExpenseOptionsContainer.style.display = "none";
    }
})
...
// Quick action: jump straight to Add tab pre-set to "Expense"
const quickAddExpenseBtn = document.querySelector(".quick_add_expense_btn");
if (quickAddExpenseBtn) {
    quickAddExpenseBtn.addEventListener("click", () => {
        getAddLink.click();
        getSelectBoxText.innerText = "Expense";
...
budgetButton.addEventListener("click", () => {
    ...
    const originalButtonText = budgetButton.innerText;
    budgetButton.disabled = true;
    budgetButton.innerText = "Updating...";

    database.postData("users/budget/update", info).then(res => {
        budgetButton.disabled = false;
        budgetButton.innerText = originalButtonText;
        ...
        elements.showToast("Budget updated successfully", "success");
...
addButton.addEventListener("click", () => {
    ...
    if (selectedType === "Expense") {
        ...
        database.postData("users/expenses/add", info).then(res => {
            ...
            addError.innerText = "";
            elements.showToast("Expense added successfully", "success");
            setTimeout(() => window.location.reload(), 700);
        }).catch(() => {
            addButton.disabled = false;
            addButton.innerText = originalButtonText;
        ...
        database.postData("users/income/add", info).then(res => {
            ...
            addError.innerText = "";
            elements.showToast("Income added successfully", "success");
            setTimeout(() => window.location.reload(), 700);
        }).catch(() => {
            addButton.disabled = false;
            addButton.innerText = originalButtonText;
