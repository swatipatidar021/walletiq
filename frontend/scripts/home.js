...
database.fetchData(`users/${userID}`).then(res => {
  ...
  budgetAmountText.innerText = `₹${formatCurrency(res?.data.budgetAmount)}`;
  ...
  // Update UI
  incomeAmount.innerText = `₹${formatCurrency(incomeSum)}`;
  expensesAmount.innerText = `₹${formatCurrency(expensesSum)}`;
